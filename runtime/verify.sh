...
: "${DF_MISE_VERSION:?run through mise so DF_MISE_VERSION is set}"
...
target=${1:-}
expected_version=$DF_JANET_VERSION
...
verify_macos() {
	...
	case "$mise_version" in "$DF_MISE_VERSION"*) ;; *) die "unexpected mise version: $mise_version" ;; esac
}
...
verify_linux_in() {
	...
	docker run --rm --platform "$docker_platform" --volume "$mise_binary:/mise:ro" "$image" /mise --version |
		grep -q "^$DF_MISE_VERSION" || die "mise failed in $image"
}
...
verify_linux() {
	...
	verify_linux_in "$binary" "$mise_binary" "$docker_platform" "$DF_ALPINE_IMAGE"
	verify_linux_in "$binary" "$mise_binary" "$docker_platform" "$DF_UBUNTU_IMAGE"
}
