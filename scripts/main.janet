...
(defn quiet-runner [argv environment]
  (def sink (file/open (if (= (get environment "DF_OS") "Windows") "NUL" "/dev/null") :w))
...
(defn command-present? [environment command]
  (def separator (if (= (get environment "DF_OS") "Windows") ";" ":"))
  (def suffix (if (= (get environment "DF_OS") "Windows") ".exe" ""))
  (var present? false)
...
(defn current-layer []
  (def target (string (selection/config-home base-environment) "/df/layer"))
  (if (os/stat target)
...
(defn print-shell-init []
  ...
  (print (string "export DF_DS=\"" ds-path "\""))
  (print "if [ -r \"${XDG_CONFIG_HOME:-$HOME/.config}/df/shell.zsh\" ]; then")
  (print "  source \"${XDG_CONFIG_HOME:-$HOME/.config}/df/shell.zsh\"")
  (print "fi")
  (print "ds() {")
  (print "  command \"$DF_DS\" \"$@\"")
  (print "  local exit_status=$?")
  (print "  case \"$1:$exit_status\" in apply:0|add:0) eval \"$(command \"$DF_DS\" shell-init)\" ;; esac")
...
(defn main [& args]
  ...
  (case command
    ...
    (do
      ...
      (put environment "DF_TARGET_USER" user)
      (def status (foreground-runner
                    [(string root "/scripts/docker-rootful.sh")
