...
usage() {
	printf '%s\n' 'usage: src/pull.sh --url URL --manifest-sha256 HEX [--prefix DIR]'
}

url=
expected_manifest_sha256=
prefix=${XDG_DATA_HOME:-$HOME/.local/share}/ds
...
[ -x "$source_dir/files/src/bootstrap.sh" ] || die 'verified bootstrap is missing'
"$source_dir/files/src/bootstrap.sh" \
	--source "$source_dir" \
	--prefix "$prefix" \
