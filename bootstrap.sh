...
usage() {
	printf '%s\n' 'usage: bootstrap.sh --source DIR [--prefix DIR] --manifest-sha256 HEX [--verify-only]'
}
...
if [ -n "$checksum_kind" ]; then
	...
	printf '%s\n' 'df bootstrap: warning: target has no SHA-256 utility; controller verification is authoritative' >&2
fi
