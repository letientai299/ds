...
runtime_dist=${DF_RUNTIME_DIST:-$root/dist/runtime}
ssh_command=${DF_SSH:-ssh}
host=${1:-}
layer=${2:-}
platform=
prefix=.local/share/df
remote_home=
dry_run=false
deliver_only=false
...
stage=$(mktemp -d "${TMPDIR:-/tmp}/df-controller.XXXXXX")
...
if [ "$deliver_only" = false ]; then
	...
	"$ssh_command" "$host" \
		"mkdir -p \"$target_home\"; HOME=\"$target_home\" XDG_CACHE_HOME=\"$target_home/.cache\" XDG_CONFIG_HOME=\"$target_home/.config\" XDG_DATA_HOME=\"$target_home/.local/share\" XDG_STATE_HOME=\"$target_home/.local/state\" MISE_CACHE_DIR=\"$target_home/.cache/mise\" MISE_CONFIG_DIR=\"$target_home/.config/df/mise\" MISE_DATA_DIR=\"$target_home/.local/share/mise\" MISE_STATE_DIR=\"$target_home/.local/state/mise\" \"$installed/ds\" apply $apply_args" >&2
fi
