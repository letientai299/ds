...
mode=${1:---check}
mise_command=${DF_MISE:-mise}
target=$root/scripts/generated/layers.janet
work=$(mktemp -d "${TMPDIR:-/tmp}/df-generate.XXXXXX")
trap 'rm -rf "$work"' EXIT HUP INT TERM

case "$mode" in --check | --write) ;; *) die 'usage: scripts/generate.sh [--check|--write]' ;; esac
command -v "$mise_command" >/dev/null 2>&1 || die 'mise is required'
command -v jq >/dev/null 2>&1 || die 'jq is required'

core=$("$mise_command" toml get --file "$root/catalog.toml" layers.core)
remote=$("$mise_command" toml get --file "$root/catalog.toml" layers.remote)
optional=$("$mise_command" toml get --file "$root/catalog.toml" layers.optional)
printf '%s\n' '{}' >"$work/components.json"
printf '%s\n%s\n%s\n' "$core" "$remote" "$optional" |
	jq -r -s 'add | unique[]' >"$work/component-names"
while IFS= read -r component; do
	commands=$("$mise_command" toml get --file "$root/catalog.toml" "components.$component.commands")
	owner=$("$mise_command" toml get --file "$root/catalog.toml" "components.$component.owner")
	tool=$("$mise_command" toml get --file "$root/catalog.toml" "components.$component.tool" 2>/dev/null || true)
