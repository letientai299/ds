...
(defn environment [base root layer]
  ...
  (put result "MISE_CONFIG_DIR" (env-path result "MISE_CONFIG_DIR" (string config-home "/df/mise")))
  ...
  (put result "MISE_SYSTEM_CONFIG_DIR" (string config-home "/df/mise-system"))
  (put result "MISE_OVERRIDE_CONFIG_FILENAMES" "mise.toml")
  (put result "MISE_GLOBAL_CONFIG_ROOT" root)
  (put result "MISE_TRUSTED_CONFIG_PATHS" root)
...
(defn binary [root environment]
  (or
    (get environment "DF_MISE")
    (let [target (platform/runtime-platform
                   {:os (platform/normalize-os (get environment "DF_OS"))
                    :arch (platform/normalize-arch (get environment "DF_ARCH"))})
          bundled (string root "/runtime/bin/" target "/mise")
...
(defn bootstrap-argv [mise root dry-run?]
  (def argv @[mise "-C" root "bootstrap" "--yes"])
  (when dry-run?
...
(defn has-tool? [runner root layer base-environment command]
  (def target-environment (environment base-environment root layer))
  (def mise (binary root target-environment))
  (= 0 (process/execute runner [mise "-C" root "which" command] target-environment)))

(defn tool-installed? [root layer base-environment tool]
  (def target-environment (environment base-environment root layer))
  (def data-dir (get target-environment "MISE_DATA_DIR"))
  (not= nil (os/stat (string data-dir "/installs/" tool))))
