...
fetch_one() {
	platform=$1
	expected=$(awk -F '\t' -v platform="$platform" '$1 == platform { print $2 }' "$root/src/runtime/mise-assets.tsv")
	url=$(awk -F '\t' -v platform="$platform" '$1 == platform { print $3 }' "$root/src/runtime/mise-assets.tsv")
...
all)
	...
	while IFS="$tab" read -r platform _checksum _url; do
		...
		fetch_one "$platform"
	done <"$root/src/runtime/mise-assets.tsv"
	;;
