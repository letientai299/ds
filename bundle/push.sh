...
usage() {
	printf '%s\n' 'usage: bundle/push.sh --snapshot DIR --host HOST [--prefix RELATIVE_PATH]'
}
...
"$ssh_command" "$host" \
	"\"$remote_source/files/bootstrap.sh\" --source \"$remote_source\" --prefix \"$remote_prefix\" --manifest-sha256 \"$manifest_sha\""
